window.dom = {
    ...
    attr (node, name, value) {
        ...
        else if (arguments.length === 3) node.getAttribute(name);
    },
    // 读写文本内容(适配)
    text (node, string) {
        if (arguments.length === 2) {
            if ("innerText" in code) node.innerText = string //IE
            ;
            else node.textContent = string //其他
            ;
        } else if (arguments.length === 1) {
            if ("innerText" in code) return node.innerText;
    ...
    style (node, name, value) {
        if (arguments.length === 3) node.style[name] = object[name];
    ...
    // 遍历所有节点
    each (nodeList, fn) {
        for(let i = 0; i < nodeList; i++)fn.call(null, nodeList[i]);
    },
    // 获取元素排行
    index (node) {
